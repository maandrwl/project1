...
class compileJava:
    ...
    def __init__(self, dictdata, PATH_MAIN, queueFolder):
        ...
        self.filenameJavac = self.filename.rsplit('.', 1)[0].lower()
        ...
        self.PATH_OUTPUT = os.path.join(PATH_MAIN, 'store_output', self.queueFolder)
        self.fStage4 = {}
    ...
    def runWork(self):
        ...
        for name_file in os.listdir(self.pathIn):
            ...
            if not self.process and self.firstprocess:
                try:
                    subprocess.run(f'javac {self.filename}', shell=True, check=True)
                    subprocess.run(f'java {self.filenameJavac} < {path_file} > {os.path.join(self.PATH_OUTPUT, name_file)}', shell=True, check=True)
                    self.process = True
